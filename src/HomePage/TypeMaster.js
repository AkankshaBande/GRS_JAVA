...
function GrievanceTypeMasterPage() {
  ...
  const handleDeleteGrievance = async (id) => {
    try {
      ...
      setGrievances(
        grievances.filter((grievance) => grievance.grievanceTypeId !== id)
      );
    } catch (error) {
    ...
    if (validateInput(trimmedGrievanceType)) {
      try {
        ...
        const response = await axios.put(
          `${API_URL}/${currentGrievance.grievanceTypeId}`,
          updatedGrievance
        );
        setGrievances(
          grievances.map((grievance) =>
            grievance.grievanceTypeId === currentGrievance.grievanceTypeId
              ? response.data
              : grievance
          )
        );
  ...
  const validateInput = (trimmedGrievanceType) => {
    ...
    if (
      grievances.some(
        (grievance) =>
          grievance.type.toLowerCase() === trimmedGrievanceType.toLowerCase()
      )
    ) {
  ...
  const filteredGrievances = grievances.filter(
    (grievance) =>
      grievance.type &&
      grievance.type.toLowerCase().includes(filter.toLowerCase())
  );
    ...
    <div className="container">
      ...
      <div className="form" style={{ margin: "5px" }}>
        ...
        <button
          ...
          onClick={
            currentGrievance ? handleUpdateGrievance : handleAddGrievance
          }
          style={{ marginTop: "5px" }}
        >
      ...
      <div className="filter">
        <input
          ...
          placeholder="Search Grievance Type"
          value={filter}
          onChange={handleFilterChange}
        />
        {filterErrorMessage && (
          <div className="invalid-feedback">{filterErrorMessage}</div>
        )}
      </div>
              ...
              <tr key={grievance.grievanceTypeId}>
                ...
                <td>
                  <button
                    ...
                    onClick={() =>
                      handleDeleteGrievance(grievance.grievanceTypeId)
                    }
                  >
                    Delete
                  </button>
